...
export default function CaseEngine({ caseType, onFinishCase, onBack }: CaseEngineProps) {
  ...
  return (
    <div className="p-6 pb-24 max-w-6xl mx-auto min-h-[88vh] flex flex-col justify-center text-white">
      {/* Simulation Header */}
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6 pb-4 border-b border-slate-800">
        <div>
          <div className="flex flex-wrap items-center gap-2 mb-1.5">
            <span className="px-3 py-1 bg-cyan-950 text-cyan-400 border border-cyan-500/30 rounded-full text-xs font-mono uppercase font-bold">
              {activeCase.difficulty}
            </span>
            <span className="px-3 py-1 bg-purple-950 text-purple-300 border border-purple-500/30 rounded-full text-xs font-mono">
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          {timeLeft !== null && (
            <div className={`flex items-center gap-2 px-4 py-2 rounded-xl border font-mono text-xs font-bold ${
      ...
      {!isCompleted && (
        <div className="flex flex-wrap gap-2 mb-6 bg-slate-900/80 p-2 rounded-2xl border border-slate-800">
          <button
          ...
          >
            <Activity size={16} /> Clinical Scenario & Decision
          </button>
          ...
          {(activeCase.initialReport || currentStep.labReport) && (
            ...
            >
              <Microscope size={16} /> Lab & Radiology Reports
            </button>
          )}
          ...
          >
            <Map size={16} /> Interactive Algorithm Map
          </button>
          ...
          {activeCase.type === 'pediatric' && (
            ...
            >
              <FileText size={16} /> Pediatric Score Tool
            </button>
          )}
        </div>
      )}
      ...
      {!isCompleted ? (
        <div>
          ...
          {activeTab === 'case' && (
            <div className="space-y-6">
              ...
              {activeCase.patient && (
                <div className="grid grid-cols-1 md:grid-cols-3 gap-5 p-5 bg-slate-900/90 border border-cyan-500/30 rounded-3xl shadow-[0_0_25px_rgba(6,182,212,0.1)]">
                  <div>
                    <p className="text-xs uppercase font-mono text-cyan-400 mb-2 flex items-center gap-1.5 font-bold">
                    ...
                    {activeCase.patient.vitalSigns && (
                      <div className="mt-3 p-2 bg-slate-950 rounded-xl text-[11px] font-mono text-slate-300 space-y-0.5 border border-slate-800">
                  ...
                  <div>
                    <p className="text-xs uppercase font-mono text-amber-400 mb-2 flex items-center gap-1.5 font-bold">
              ...
              <div className="bg-slate-900/60 p-4 rounded-2xl border border-slate-800">
                <h2 className="text-lg font-bold text-cyan-300 mb-1">
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center gap-4">
                        <div className="w-8 h-8 rounded-xl bg-slate-800 flex items-center justify-center font-mono text-xs font-bold text-cyan-400 border border-slate-700 shrink-0 group-hover:border-cyan-400">
                          {opt.id.slice(-1).toUpperCase()}
                        </div>
                        <span className="font-medium text-sm"><HighlightedText text={opt.label} /></span>
                      </div>

                      {isSelected && (
                        <div>
                          {opt.isCorrect ? <CheckCircle className="text-emerald-400" size={24} /> : <XCircle className="text-rose-400" size={24} />}
                        </div>
                      )}
                    </button>
                  );
              ...
              <AnimatePresence>
                {feedback && (
                  ...
                  >
                    ...
                    <div className="p-3 bg-slate-950/80 rounded-xl border border-slate-800 text-[11px] font-mono text-cyan-300 flex items-center gap-2 mb-4">
                    ...
                    <button
                      ...
                      className="px-6 py-3 bg-gradient-to-r from-cyan-500 to-blue-600 hover:from-cyan-400 hover:to-blue-500 text-white font-bold rounded-xl text-xs flex items-center gap-2 shadow-[0_0_20px_rgba(6,182,212,0.3)] transition-all"
                    >
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                {(activeCase.initialReport?.smearMicroscopy || currentStep.labReport?.smearMicroscopy) && (
                  <div className="p-5 bg-slate-900 border border-slate-800 rounded-2xl space-y-3">
                    <div className="flex justify-between items-center pb-2 border-b border-slate-800">
                      <span className="font-mono text-xs text-purple-400 font-bold uppercase">Sputum Smear Microscopy</span>
                ...
                {(activeCase.initialReport?.cbnaat || currentStep.labReport?.cbnaat) && (
                  <div className="p-5 bg-slate-900 border border-cyan-500/40 rounded-2xl space-y-3 shadow-[0_0_15px_rgba(6,182,212,0.1)]">
                    <div className="flex justify-between items-center pb-2 border-b border-slate-800">
                      <span className="font-mono text-xs text-cyan-400 font-bold uppercase">CBNAAT / Xpert MTB/RIF Report</span>
                ...
                {(activeCase.initialReport?.chestXray || currentStep.labReport?.chestXray) && (
                  <div className="p-5 bg-slate-900 border border-slate-800 rounded-2xl space-y-3 md:col-span-2">
                    <div className="flex justify-between items-center pb-2 border-b border-slate-800">
                      <span className="font-mono text-xs text-indigo-400 font-bold uppercase">Chest Radiograph (CXR) Report</span>
        ...
        <motion.div
          ...
          className="p-8 bg-slate-900 border border-cyan-500/40 rounded-3xl text-center space-y-6 shadow-[0_0_50px_rgba(6,182,212,0.2)] max-w-xl mx-auto"
        >
          <div className="inline-flex p-4 bg-cyan-950 border border-cyan-500/40 rounded-full text-cyan-400 mb-2 shadow-inner">
            <Trophy size={48} />
          </div>
          <h2 className="text-3xl font-extrabold text-white">Clinical Simulation Mastered!</h2>
          ...
          <div className="grid grid-cols-3 gap-3 p-4 bg-slate-950 rounded-2xl border border-slate-800 text-center font-mono">
            <div>
              ...
              <p className="text-xl text-cyan-400 font-bold">{score + 150}</p>
            </div>
            <div>
              <p className="text-[10px] text-slate-500 uppercase">XP Earned</p>
              <p className="text-xl text-amber-400 font-bold">+{xpEarned + 200} XP</p>
            </div>
            <div>
              <p className="text-[10px] text-slate-500 uppercase">Time Elapsed</p>
              <p className="text-xl text-emerald-400 font-bold">{timeTaken}s</p>
            </div>
          </div>

          <div className="p-4 bg-slate-950/60 rounded-2xl border border-slate-800 text-left text-xs space-y-2 font-mono">
          ...
          <button
            ...
            className="w-full py-4 bg-gradient-to-r from-cyan-500 to-blue-600 hover:from-cyan-400 hover:to-blue-500 text-white font-bold text-sm rounded-xl shadow-[0_0_25px_rgba(6,182,212,0.3)] transition-all"
          >
            Return to Dashboard
          </button>
        </motion.div>
